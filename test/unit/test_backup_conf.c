...
assert_bup_config_eq(backup_config_t *c1, backup_config_t *c2)
{
	...
	CMP_STR_FIELD(c1->prefix, c2->prefix);
	...
	CMP_INT_FIELD(c1->estimate, c2->estimate);
 ...
 * by mult before comparing to the parsed backup_config
 */
...
START_TEST(test_init_set_list)
{
	...
	as_vector_append(&c2.set_list, "set-1");
	as_vector_append(&c2.set_list, "set-2");
	as_vector_append(&c2.set_list, "set-3");
...
START_TEST(test_init_mod_after)
{
	...
	c2.mod_after = mktime(&t) * 1000000000l;
...
START_TEST(test_init_mod_before)
{
	...
	c2.mod_before = mktime(&t) * 1000000000l;
...
Suite* backup_conf_suite()
{
	...
	s = suite_create("Backup config file");
	...
	tcase_add_test(tc_init, test_init_estimate);
