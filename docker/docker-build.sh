...
#
# Build multi-architecture Docker images for Aerospike asbackup.
#
# Flow:
...
Build the Aerospike asbackup Docker image (single recipe; multi-arch).
...
REQUIRED:
    -v, --version VERSION   asbackup version, e.g. 0.20.1 or 0.20.1-rc1

                            A "rcN" pre-release tag is NOT part of any name: it
                            becomes the package iteration, so 0.20.1-rc2 resolves
                            the .deb 0.20.1-2<distro> and tags images 0.20.1-2.
...
TAGS PRODUCED:
  <tag> below is <version-without-rcN>-<iteration>, e.g. 0.20.1-rc2 -> 0.20.1-2
  and 0.20.1 -> 0.20.1-1.

  Test mode (loaded locally):
    <reg>/aerospike-asbackup:<tag>-<arch>
    e.g. aerospike/aerospike-asbackup:5.0.0-1-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-asbackup:<tag>
    <reg>/aerospike-asbackup:<tag>-<timestamp>   (always; default timestamp is current UTC; -T overrides)
...
EXAMPLES:
    ...
    docker/docker-build.sh -t -v 0.20.1-rc1 -u /path/to/dist

    # Build + load from JFrog dev (single -u sets the DEB base URL)
    docker/docker-build.sh -t -v 0.20.1-rc1 \
    ...
    # Build + push to default registry (docker.io/aerospike/aerospike-asbackup)
...
function _emit_test_target() {
  ...
  if [[ "${arch}" == "amd64" ]]; then
    _emit_args \
      ASBACKUP_AMD64_URL "${ASBACKUP_AMD64_URL}" \
      ASBACKUP_AMD64_SHA256 "${ASBACKUP_AMD64_SHA256}" \
      ASBACKUP_LOCAL_PKG "${local_pkg}"
  else
    _emit_args \
      ASBACKUP_ARM64_URL "${ASBACKUP_ARM64_URL}" \
      ASBACKUP_ARM64_SHA256 "${ASBACKUP_ARM64_SHA256}" \
      ASBACKUP_LOCAL_PKG "${local_pkg}"
  fi
  _emit_tags "${tags[@]}"
  echo "}"
...
function _emit_push_target() {
  ...
  _emit_args \
    ...
    ASBACKUP_ARM64_URL "${ASBACKUP_ARM64_URL}" \
    ASBACKUP_ARM64_SHA256 "${ASBACKUP_ARM64_SHA256}" \
    ASBACKUP_LOCAL_PKG_AMD64 "${local_pkg_amd64}" \
    ASBACKUP_LOCAL_PKG_ARM64 "${local_pkg_arm64}"
  _emit_attestations
  _emit_tags "${tags[@]}"
  echo "}"
  echo ""
...
function _emit_single_arch_push_target() {
  ...
  if [[ "${arch}" == "amd64" ]]; then
    _emit_args \
      ASBACKUP_AMD64_URL "${ASBACKUP_AMD64_URL}" \
      ASBACKUP_AMD64_SHA256 "${ASBACKUP_AMD64_SHA256}" \
      ASBACKUP_LOCAL_PKG_AMD64 "${local_pkg}"
  else
    _emit_args \
      ASBACKUP_ARM64_URL "${ASBACKUP_ARM64_URL}" \
      ASBACKUP_ARM64_SHA256 "${ASBACKUP_ARM64_SHA256}" \
      ASBACKUP_LOCAL_PKG_ARM64 "${local_pkg}"
  fi
  _emit_attestations
  _emit_tags "${tags[@]}"
  echo "}"
...
# Requires global IMAGE_TAG, REGISTRY_PREFIXES
# ---------------------------------------------------------------------------
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-asbackup:${IMAGE_TAG}-amd64"
    src_arm64="${reg}/aerospike-asbackup:${IMAGE_TAG}-arm64"
    target_tag="${reg}/aerospike-asbackup:${IMAGE_TAG}"
    log_info "imagetools create ${target_tag}"
    local create_args=(-t "${target_tag}")
    if [[ "${MOVING_TAGS}" == true ]]; then
      for mt in $(moving_tags); do create_args+=(-t "${reg}/aerospike-asbackup:${mt}"); done
    fi
...
# Reads globals: IMAGE_TAG, REGISTRY_PREFIXES, ACTIVE_ARCHES, PUSH_SINGLE_ARCH,
#                ASBACKUP_*_URL, ASBACKUP_*_SHA256, LOCAL_PKG_*
# ---------------------------------------------------------------------------
function generate_bake() {
...

  {
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asbackup:${IMAGE_TAG}-${arch}")
      done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asbackup:${IMAGE_TAG}")
        if [[ -n "${TIMESTAMP}" ]]; then
          tags+=("${reg}/aerospike-asbackup:${IMAGE_TAG}-${TIMESTAMP}")
        fi
        if [[ "${MOVING_TAGS}" == true ]]; then
          for mt in $(moving_tags); do tags+=("${reg}/aerospike-asbackup:${mt}"); done
        fi
      done
...
#   ASBACKUP_{AMD64,ARM64}_URL
#   ASBACKUP_{AMD64,ARM64}_SHA256
...
function resolve_packages() {
  local pkg_amd64="aerospike-asbackup_${PKG_VERSION}-${ITERATION}ubuntu24.04_amd64.deb"
  local pkg_arm64="aerospike-asbackup_${PKG_VERSION}-${ITERATION}ubuntu24.04_arm64.deb"
  local pool="${DEB_BASE_URL}/pool/${UBUNTU_CODENAME}/aerospike-asbackup"

  ASBACKUP_AMD64_URL="${pool}/${pkg_amd64}"
  ASBACKUP_ARM64_URL="${pool}/${pkg_arm64}"
  ...
  if [[ -n "${PACKAGES_DIR}" ]]; then
    ...
    if [[ "${need_amd64}" == true ]]; then
      ...
      if [[ -n "${path}" ]]; then
        log_info "  Found:  ${path}"
        ASBACKUP_AMD64_SHA256=$(sha256_of_file "${path}")
        ...
        ASBACKUP_AMD64_SHA256="PLACEHOLDER"
      fi
    fi
    if [[ "${need_arm64}" == true ]]; then
      ...
      if [[ -n "${path}" ]]; then
        log_info "  Found:  ${path}"
        ASBACKUP_ARM64_SHA256=$(sha256_of_file "${path}")
        ...
        ASBACKUP_ARM64_SHA256="PLACEHOLDER"
      fi
    fi
  elif [[ "${COMPUTE_SHA}" == true ]]; then
    [[ "${need_amd64}" == true ]] && ASBACKUP_AMD64_SHA256=$(sha256_of_url "${ASBACKUP_AMD64_URL}")
    [[ "${need_arm64}" == true ]] && ASBACKUP_ARM64_SHA256=$(sha256_of_url "${ASBACKUP_ARM64_URL}")
  else
    ASBACKUP_AMD64_SHA256="PLACEHOLDER"
    ASBACKUP_ARM64_SHA256="PLACEHOLDER"
  fi
  ...
  echo ""
  log_info "  AMD64 URL: ${ASBACKUP_AMD64_URL}"
  log_info "  AMD64 SHA: ${ASBACKUP_AMD64_SHA256}"
  log_info "  ARM64 URL: ${ASBACKUP_ARM64_URL}"
  log_info "  ARM64 SHA: ${ASBACKUP_ARM64_SHA256}"
  if [[ "${ASBACKUP_AMD64_SHA256}" == "PLACEHOLDER" || "${ASBACKUP_ARM64_SHA256}" == "PLACEHOLDER" ]]; then
...
ASBACKUP_AMD64_URL=""
ASBACKUP_AMD64_SHA256=""
ASBACKUP_ARM64_URL=""
ASBACKUP_ARM64_SHA256=""
