...
TAGS PRODUCED:
  Test mode (loaded locally):
    <reg>/aerospike-asbackup:<version>-<arch>
    e.g. aerospike/aerospike-asbackup:5.0.0-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-asbackup:<version>
    <reg>/aerospike-asbackup:<version>-<timestamp>   (always; default timestamp is current UTC; -T overrides)
...
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-asbackup:${VERSION}-amd64"
    src_arm64="${reg}/aerospike-asbackup:${VERSION}-arm64"
    target_tag="${reg}/aerospike-asbackup:${VERSION}"
...
function generate_bake() {
...

  {
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asbackup:${VERSION}-${arch}")
      done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asbackup:${VERSION}")
        if [[ -n "${TIMESTAMP}" ]]; then
          tags+=("${reg}/aerospike-asbackup:${VERSION}-${TIMESTAMP}")
        fi
      done
...
function resolve_packages() {
  local pkg_amd64="aerospike-asbackup_${VERSION}-1_ubuntu24.04_x86_64.deb"
  local pkg_arm64="aerospike-asbackup_${VERSION}-1_ubuntu24.04_aarch64.deb"
