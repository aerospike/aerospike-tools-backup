...
@test "build debian12 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="debian12" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-debian12-$VERSION"
...
@test "build ubuntu20.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu20.04" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-ubuntu20.04-$VERSION"
...
@test "build ubuntu22.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu22.04" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-ubuntu22.04-$VERSION"
...
@test "build ubuntu24.04 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="ubuntu24.04" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-ubuntu24.04-$VERSION"
...
@test "build redhat ubi9 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="redhat-ubi9" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-redhat-ubi9-$VERSION"
  [ $? -eq 0 ] && [ -f $TEMP_DIR/redhat-ubi9/*.deb ]
}

@test "build redhat ubi10" {
...
@test "build redhat ubi10 package" {
  TEMP_DIR=$(mktemp -d)
  docker run -e BUILD_DISTRO="redhat-ubi10" -v $TEMP_DIR:/tmp/output "asbackup-pkg-builder-redhat-ubi10-$VERSION"
